...
A key feature of GooFit is the indexing system; this is the system GooFit uses to allow the PDF and amplitude code access to shared values. The original system, shown in figure~\ref{fig-indexing-classic}, just stores values once, but then uses a double lookup system to find the correct index to read. The new system, shown in figure~\ref{fig-indexing-new}, uses a little preprocessing to store values in a structure that no longer requires a double lookup, saving a significant amount of time in some cases.
